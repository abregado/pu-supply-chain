...
quest_gui.init = function(force)
  print("quest_gui init")
  global.quest_controller_data = quest_gui_data
  global.quest_controller_data.default_force = force or game.forces.player 
end
...
quest_gui.set = function(quest_name, tree_view_rows, force)
  if force == nil then force = quest_gui_data.default_force end
  quest_gui.unset(force)
...
quest_gui.add_hint = function(hint, force)
  if force == nil then force = quest_gui_data.default_force end
...
quest_gui.visible = function(state,force)
  if force == nil then force = quest_gui_data.default_force end
  for _, player in pairs(force.players) do
    local objective_flow = player.gui.left.objective_flow
    if objective_flow then
...
quest_gui.unset = function(force)
  if force == nil then force = quest_gui_data.default_force end
  for _, player in pairs(force.players) do
    local objective_flow = player.gui.left.objective_flow
    if objective_flow then
...
quest_gui.update_count = function(item_name, count, goal,force)
  if force == nil then force = quest_gui_data.default_force end
  for _, player in pairs(force.players) do
...
quest_gui.update_state = function(item_name, state,force)
  if force == nil then force = quest_gui_data.default_force end
  for _, player in pairs(force.players) do
    local item_gui = get_item_gui(player, item_name)
...
quest_gui.migrate = function(force)
  if force == nil then force = quest_gui_data.default_force end
...
quest_gui.on_player_joined_game = on_player_joined_game
quest_gui.on_gui_click = on_gui_click
